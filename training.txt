...
import json
import pickle
# Must install library via command line
import numpy as np
# Must install library via command line
import nltk
...
def train_model():
    ...
    # will hold a complete list of patterns ('patterns' in json)
    words = []
    topics = []            # hold a list of all the topics in the json file
    # dictonary of topics and their coresponding list of keywords (topics/keyword)
    docs = []
    ignore_characters = ['?', '!', '.', ',']

    # parse through the data and save it into lists
    for intent in data["intents"]:                      # parse every "tag" chunk
        # grabs list of words in "patterns" of current "topic"
        for keyword in intent["patterns"]:
            word_List = nltk.word_tokenize(keyword)     # tokenize words
            # add it to a complete list of words
            words.extend(word_List)
            # assigns current list of words to the current "topic" (greetings = hello, hey, whats up, ...)
            docs.append((word_List, intent["tag"]))

        # saves list of "topics"
        # (topics = greetings, bye, advising, ...)
        if intent["tag"] not in topics:
            topics.append(intent["tag"])

    # lematize (stem) "words" and remove duplicates in lists 'words" and "topics"
    words = [lemmatizer.lemmatize(w.lower())
            for w in words if w not in ignore_characters]
    ...
    # CHECKPOINT:
    #   So far, we have a complete List-of-root-Words that function like patterns.          ("words" list)
    #   We also have a List-of-topics which are the header of each chunk in the json file.    ("topics" list)
    #   We also have a list that connect a topic with it corresponding List-of-patterns       ("doc" list)
    ...
    training = []
    # inital binary list of topics (topics)
    output_Empty = [0] * len(topics)

    for document in docs:                       # create bag of words
        bag = []
        # document[0] = the list of key words of current topic
        word_keyword = document[0]
        word_keyword = [lemmatizer.lemmatize(w.lower()) for w in word_keyword]

        # compare each word in complete list of patterns to current topic's list of patterns
        for w in words:
    ...
    myModel = model.fit(np.array(training_X), np.array(
